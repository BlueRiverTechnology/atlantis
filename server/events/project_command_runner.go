...
//go:generate pegomock generate --package mocks -o mocks/mock_job_url_generator.go JobURLGenerator

// JobURLGenerator generates urls to project job streams.
type JobURLGenerator interface {
	// GenerateProjectJobURL returns the full URL to view the job output.
...
func (p *DefaultProjectCommandRunner) Plan(ctx command.ProjectContext) command.ProjectResult {
	...
	var jobURL string
	if p.JobURLGenerator != nil {
		jobURL, _ = p.JobURLGenerator.GenerateProjectJobURL(ctx)
	}
...
func (p *DefaultProjectCommandRunner) Apply(ctx command.ProjectContext) command.ProjectResult {
	...
	// Attempt to read the JSON output file if it was generated by the 'show' step
	var applySummary string
	if err == nil && failure == "" {
		// Only try to read JSON if apply succeeded
		repoDir, repoErr := p.WorkingDir.GetWorkingDir(ctx.Pull.BaseRepo, ctx.Pull, ctx.Workspace)
		if repoErr == nil {
			projAbsPath := filepath.Join(repoDir, ctx.RepoRelDir)
			jsonFileName := ctx.GetShowResultFileName()
			jsonFilePath := filepath.Join(projAbsPath, jsonFileName)
			if jsonData, readErr := os.ReadFile(jsonFilePath); readErr == nil {
				applyJSON := string(jsonData)
				ctx.Log.Debug("successfully read apply JSON output from %s", jsonFileName)
				// Generate human-readable summary instead of returning raw JSON
				applySummary = GenerateResourceChangeSummary(applyJSON)
				ctx.Log.Debug("generated apply summary with %d characters", len(applySummary))
			} else if !os.IsNotExist(readErr) {
				// Log non-NotExist errors but don't fail the apply
				ctx.Log.Warn("error reading apply JSON file %s: %v", jsonFileName, readErr)
			}
		}
	}
...
func (p *DefaultProjectCommandRunner) doPlan(ctx command.ProjectContext) (*models.PlanSuccess, string, error) {
	...
	jsonFilePath := filepath.Join(projAbsPath, jsonFileName)
	if jsonData, readErr := os.ReadFile(jsonFilePath); readErr == nil {
		planJSON := string(jsonData)
		ctx.Log.Debug("successfully read plan JSON output from %s", jsonFileName)
		// Generate human-readable summary instead of returning raw JSON
		planSummary = GenerateResourceChangeSummary(planJSON)
		ctx.Log.Debug("generated plan summary with %d characters", len(planSummary))
	} else if !os.IsNotExist(readErr) {
		// Log non-NotExist errors but don't fail the plan
		ctx.Log.Warn("error reading plan JSON file %s: %v", jsonFileName, readErr)
	}
	// If file doesn't exist (show step not configured), planSummary remains empty
